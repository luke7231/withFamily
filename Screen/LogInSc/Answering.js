...
const ANSWRING = gql`
    mutation answering($payload:String,$file:Upload,$questionId:Int!){
        answering(payload:$payload, file:$file, questionId:$questionId){
            ok
            error
        }
    }
...
export default function Answering({ route, navigation }) {
    ...
    useEffect(() => {
        register("answering", { required: true })
    }, [register])
    ...
    const [answeringMutation, { data, loading }] = useMutation(ANSWRING, {
        onCompleted: ({ answering: { ok, error } }) => {
            if (ok) {
                navigation.navigate("가족 질문함");
                sendNotification(pushTokenArr,`${route.params.username}님이 답변을 달았습니다!`)
            }
        },
        refetchQueries: [{ query: SEE_QUESTION_LIST }]
    })
    const onValid = ({ answering }) => {
        let file
        if (image) {
                    ...
                    <ImageBackground blurRadius={0.2} source={require("../../assets/배경.png")} resizeMode='cover' style={{ flex: 1, height, width, justifyContent: "center", alignItems: "center" }}>
                        ...
                        <View testID='답변박스' style={{ width: CARDWIDTH, height: CARDHEIGHT, backgroundColor: "white", marginBottom: 100 }}>
                            <View testID='프로필상단박스' style={{ backgroundColor: "skyblue", width: CARDWIDTH, height: PROFILEBOXHEIGHT, justifyContent: "space-around", alignItems: "center", flexDirection: "row" }}>
                                ...
                                <TouchableOpacity onPress={handleSubmit(onValid)} style={{ width: 70, height: 30, justifyContent: "center", alignItems: "center",borderRadius:7, backgroundColor: "white" }}>
                                    {loading ? <ActivityIndicator size={"small"} color="skyblue"/> : <Text style={{ color: "skyblue", fontFamily: galaxy ? null : 'font' }}>답변달기</Text>}
                                    </TouchableOpacity>
                            </View>
                            <View testID='페이로드박스' style={{ backgroundColor: "white", width: CARDWIDTH, height: CARDHEIGHT * 0.85, paddingTop: 30, alignItems: "center" }}>
                                {image && <Image source={{ uri: image }} style={{ width: IMAGEHEIGHT*0.8, height: IMAGEHEIGHT*0.8 }} />}
                                <TextInput
